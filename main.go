...
func (s *State) run(w *app.Window) error {
	...
	var ops op.Ops
	const pingDuration = 500 * time.Millisecond
	timer := time.NewTimer(pingDuration)
	for {
		select {
		case <-timer.C:
			timer.Reset(pingDuration)
			if s.needRefresh && time.Now().After(s.refreshTime) {
				s.needRefresh = false
				s.was = s.now
				s.now = StateOne{bg: rgb(0, 0, 0), fg: rgb(255, 255, 255), msg: "click"}
		...
		case e := <-w.Events():
			...
			case system.FrameEvent:
				...
				if released {
					...
					s.clickTime = time.Now()
					s.tloc = ploc
					s.refreshTime = s.clickTime.Add(refreshDuration)
					s.needRefresh = true
				}
